# -*- coding: utf-8 -*-
"""SSDB-compatible hash adapter with the same protocol semantics as Redis."""
...
class SsdbClient(object):
    """The upstream SSDB adapter uses redis-py commands, so share semantics."""
    ...
    def _matches(cls, value, https=False, protocol=None):
        ...
        if https and not data.get("https"):
            return False
        return protocol is None or cls._protocol(value) == normalize_protocol(protocol)

    def _values(self):
        return list(self.__conn.hgetall(self.name).values())
    ...
    def getCount(self):
        ...
        https_count = 0
        for value in values:
            protocol_count[self._protocol(value)] += 1
